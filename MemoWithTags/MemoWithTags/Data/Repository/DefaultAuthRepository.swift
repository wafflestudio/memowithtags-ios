...
final class DefaultAuthRepository: AuthRepository {
    ...
    func sendEmail(email: String) async throws {
        print("🙏 send email")
        let response = await AF.request(AuthRouter.sendEmail(email: email)).serializingData(emptyResponseCodes: [204]).response
    ...
    func verifyEmail(email: String, code: String) async throws {
        print("🙏 verify email")
        let response = await AF.request(AuthRouter.verifyEmail(email: email, code: code)).serializingData(emptyResponseCodes: [204]).response
        try handleError(response: response)
    }
    ...
    func register(nickname: String, email: String, password: String) async throws -> AuthDto {
        print("🙏 register")
        let response = await AF.request(AuthRouter.register(nickname: nickname, email: email, password: password)).serializingDecodable(AuthDto.self).response
        let dto = try handleErrorDecodable(response: response)
        return dto
    }
    ...
    func resetPassword(email:String, newPassword: String) async throws {
        ...
        let response = await AF.request(AuthRouter.resetPassword(email: email, newPassword: newPassword)).serializingData(emptyResponseCodes: [204]).response
        try handleError(response: response)
    }
    ...
    func changeNickname(nickname: String) async throws {
        ...
        let response = await AF
            .request(AuthRouter.changeNickname(nickname: nickname), interceptor: tokenInterceptor).serializingDecodable(UserDto.self).response
    ...
    func withdrawal(email: String) async throws {
        print("🙏 withdrawal")
        let response = await AF.request(AuthRouter.withdrawal(email: email), interceptor: tokenInterceptor).serializingData(emptyResponseCodes: [204]).response
        try handleError(response: response)
    }
